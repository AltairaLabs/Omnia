...
export function useAgent(name: string, _namespace?: string) {
  ...
  return useQuery({
    ...
    staleTime: DEFAULT_STALE_TIME,
    refetchOnMount: true, // Only refetch if stale
    // Rollout live-updates come from the SSE stream (use-agent-rollout-stream),
    // not client polling — that's what catches a rollout *starting* and lingers
    // the terminal state. See Omnia#1235.
  });
}
