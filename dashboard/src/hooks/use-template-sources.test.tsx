...
function createMockSource(overrides: Partial<ArenaTemplateSource> = {}): ArenaTemplateSource {
  return {
    ...
    status: {
      ...
      templateCount: 2,
      templates: [
        { name: "template-1", path: "templates/t1" },
        { name: "template-2", path: "templates/t2" },
      ],
    },
    ...overrides,
  };
    ...
    it("aggregates templates from all ready sources", async () => {
      const sources = [
        createMockSource({
          ...
          status: {
            phase: "Ready",
            templates: [createMockTemplate({ name: "t1" })],
          },
        }),
        createMockSource({
          ...
          status: {
            phase: "Ready",
            templates: [createMockTemplate({ name: "t2" })],
          },
        }),
      ];
      // The API returns the array directly, not wrapped in { sources }
      ...
      expect(result.current.templates[0].sourceName).toBe("source-1");
      expect(result.current.templates[1].sourceName).toBe("source-2");
    });
