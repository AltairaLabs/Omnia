...
function MessageBubble({ message, showTimestamp, evalResults, syntheticUser }: Readonly<{ message: Message; showTimestamp?: boolean; evalResults?: EvalResult[]; syntheticUser?: SyntheticUser | null }>) {
    ...
    <div className={cn("flex gap-3", isUser ? "flex-row-reverse" : "flex-row")}>
      ...
      <div className={cn("flex flex-col max-w-[80%]", isUser ? "items-end" : "items-start")}>
        ...
        <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
          {isUser && syntheticUser && (
            <Badge
              variant="secondary"
              className="gap-1 font-normal"
              title="This user turn was generated by the arena, not a real user"
            >
              <Bot className="h-3 w-3" />
              {syntheticUserLabel(syntheticUser)}
            </Badge>
          )}
          {showTimestamp && (
...
function ConversationMessages({
  ...
  onLoadMore?: () => void;
  syntheticUser?: SyntheticUser | null;
}>) {
...
function ConversationWithDebugPanel({
  ...
  const syntheticUser = syntheticUserInfo(session.metadata?.tags);
