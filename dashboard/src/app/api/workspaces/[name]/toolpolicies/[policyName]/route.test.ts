/**
 * Tests for individual workspace-scoped tool policy API routes.
 */
...
function createMockContext() {
  return {
    params: Promise.resolve({ name: "test-ws", policyName: "pii-guard" }),
  };
}
...
describe("GET /api/workspaces/[name]/toolpolicies/[policyName]", () => {
  ...
  it("returns tool policy for authenticated user with access", async () => {
  ...
  it("returns 404 when tool policy not found", async () => {
    const { getUser } = await import("@/lib/auth");
    const { checkWorkspaceAccess } = await import("@/lib/auth/workspace-authz");
    ...
    vi.mocked(getWorkspaceResource).mockResolvedValue({
      ok: false,
      response: notFoundResponse("Tool policy not found: pii-guard"),
    });
...
describe("PUT /api/workspaces/[name]/toolpolicies/[policyName]", () => {
  ...
  it("updates tool policy for user with editor role", async () => {
    ...
    const updatedPolicy = {
      ...
      spec: {
        selector: { registry: "data-tools" },
        rules: [{ name: "no-pii", deny: { cel: "has(params.ssn) || has(params.creditCard)", message: "PII not allowed" } }],
        mode: "audit",
      },
    };
    ...
    const requestBody = { spec: { selector: { registry: "data-tools" }, rules: [], mode: "audit" } };
    const response = await PUT(createMockRequest("PUT", requestBody), createMockContext());

    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body.spec.mode).toBe("audit");
  });
