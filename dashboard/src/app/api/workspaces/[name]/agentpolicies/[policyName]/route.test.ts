/**
 * Tests for individual workspace-scoped agent policy API routes.
 */
...
const mockPolicy = {
  ...
  spec: { toolAccess: { mode: "allowlist", rules: [{ registry: "search-tools", tools: ["web-search"] }] } },
};

function createMockRequest(method: string, body?: unknown): NextRequest {
  const url = "http://localhost:3000/api/workspaces/test-ws/agentpolicies/default-policy";
...
function createMockContext() {
  return {
    params: Promise.resolve({ name: "test-ws", policyName: "default-policy" }),
  };
}
...
describe("GET /api/workspaces/[name]/agentpolicies/[policyName]", () => {
  ...
  it("returns agent policy for authenticated user with access", async () => {
    ...
    expect(body.metadata.name).toBe("default-policy");
  });
  ...
  it("returns 404 when agent policy not found", async () => {
    const { getUser } = await import("@/lib/auth");
    const { checkWorkspaceAccess } = await import("@/lib/auth/workspace-authz");
    ...
    vi.mocked(getWorkspaceResource).mockResolvedValue({
      ok: false,
      response: notFoundResponse("Agent policy not found: default-policy"),
    });
...
describe("PUT /api/workspaces/[name]/agentpolicies/[policyName]", () => {
  ...
  it("updates agent policy for user with editor role", async () => {
    ...
    const updatedPolicy = {
      ...mockPolicy,
      spec: { toolAccess: { mode: "denylist", rules: [] } },
    };
    vi.mocked(updateCrd).mockResolvedValue(updatedPolicy);

    const { PUT } = await import("./route");
    const requestBody = { spec: { toolAccess: { mode: "denylist", rules: [] } } };
