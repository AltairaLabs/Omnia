/**
 * Tests for individual workspace-scoped tool registry API routes.
 */
...
const mockRegistry = {
  metadata: { name: "my-tools", namespace: "test-ns" },
  spec: { tools: [] },
};

function createMockRequest(method: string, body?: unknown): NextRequest {
  const url = "http://localhost:3000/api/workspaces/test-ws/toolregistries/my-tools";
...
function createMockContext() {
  return {
    params: Promise.resolve({ name: "test-ws", registryName: "my-tools" }),
  };
}
...
describe("GET /api/workspaces/[name]/toolregistries/[registryName]", () => {
  ...
  it("returns tool registry for authenticated user with access", async () => {
    ...
    expect(body.metadata.name).toBe("my-tools");
  });
  ...
  it("returns 404 when tool registry not found", async () => {
    ...
    vi.mocked(getWorkspaceResource).mockResolvedValue({
      ok: false,
      response: notFoundResponse("Tool registry not found: my-tools"),
    });
