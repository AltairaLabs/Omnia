 ...
 *   → PRIVACY_API_URL/api/v1/privacy/consent/stats?workspace={uid}
 *
 * Aggregate-only — does NOT expose individual user consent (that lives at
 * /api/workspaces/{name}/privacy/consent). Handler moved from memory-api to
 * privacy-api in Slice B (#1642).
 */
...
export const GET = withWorkspaceAccess(
  ...
  async (
    ...
    const targetUrl = `${base}/api/v1/privacy/consent/stats?workspace=${encodeURIComponent(uid)}`;
    ...
    try {
      ...
      console.error("Consent stats API proxy error:", error);
      return NextResponse.json(
        {
