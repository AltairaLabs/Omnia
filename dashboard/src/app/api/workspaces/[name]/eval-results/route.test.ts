/**
 * Tests for eval results listing proxy route.
 *
...
function createMockRequest(query = ""): NextRequest {
  const url = `http://localhost:3000/api/workspaces/test-ws/eval-results${query}`;
...
describe("GET /api/workspaces/[name]/eval-results", () => {
  ...
  it("proxies eval-results list to backend", async () => {
    ...
    const evalResults = [
      { id: "e1", sessionId: "s1", evalId: "tone", passed: false },
    ];
    mockFetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ evalResults, total: 1 }),
    });
  ...
  it("forwards filter query params", async () => {
    ...
    const { GET } = await import("./route");
    await GET(createMockRequest("?agentName=myagent&passed=false&limit=10"), createMockContext());

    const fetchUrl = mockFetch.mock.calls[0][0] as string;
    expect(fetchUrl).toContain("agentName=myagent");
    expect(fetchUrl).toContain("passed=false");
