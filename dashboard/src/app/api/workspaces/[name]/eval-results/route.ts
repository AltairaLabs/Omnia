/**
 * Eval results listing proxy route.
 *
 * GET /api/workspaces/{name}/eval-results
 *   → SESSION_API_URL/api/v1/eval-results?namespace={name}&...
 *
  ...
  async (
    ...
    if (!SESSION_API_URL) {
      return NextResponse.json(
        { error: "Session API not configured", evalResults: [], total: 0 },
        { status: 503 }
      );
    }
    ...
    const forwardParams = ["agentName", "evalId", "evalType", "passed", "limit", "offset"];
    ...
    const targetUrl = `${baseUrl}/api/v1/eval-results?${params.toString()}`;
