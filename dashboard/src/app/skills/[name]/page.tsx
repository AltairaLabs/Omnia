...
function sourceUrl(
  source: ReturnType<typeof useSkillSource>["source"]
): string {
  if (!source) return "";
  if (source.spec.type === "git" && source.spec.git) {
    return source.spec.git.url;
  }
  if (source.spec.type === "oci" && source.spec.oci) {
    return source.spec.oci.url;
  }
      ...
      <div className="flex-1 p-6 space-y-4 overflow-auto">
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            {filter && (filter.include?.length || filter.exclude?.length || filter.names?.length) ? (
              <div>
                <div className="text-muted-foreground text-xs uppercase">
                  Filter
                </div>
