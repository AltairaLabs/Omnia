...
describe("TemplateBrowser", () => {
  it("renders templates count", () => {
    ...
    render(<TemplateBrowser sources={sources} />);
    expect(screen.getByText(/2 templates available/)).toBeInTheDocument();
  });

  it("displays templates from ready sources", () => {
    const sources = [createMockSource()];
    render(<TemplateBrowser sources={sources} />);
  ...
  it("skips templates from non-ready sources", () => {
    const sources = [
      createMockSource({
        status: {
          phase: "Error",
    ...
    render(<TemplateBrowser sources={sources} />);
    expect(screen.queryByText("Error Template")).not.toBeInTheDocument();
  ...
  it("shows error state", () => {
    ...
    render(<TemplateBrowser sources={[]} error={error} />);
  ...
  it("shows retry button in error state when onRefetch is provided", () => {
    ...
    render(<TemplateBrowser sources={[]} error={error} onRefetch={onRefetch} />);
  ...
  it("shows loading skeletons when loading", () => {
    render(<TemplateBrowser sources={[]} loading />);
  ...
  it("shows empty state when no templates", () => {
    render(<TemplateBrowser sources={[]} />);
  ...
  it("calls onRefetch when refresh button is clicked", () => {
    const onRefetch = vi.fn();
    const sources = [createMockSource()];
    render(<TemplateBrowser sources={sources} onRefetch={onRefetch} />);
  ...
  it("calls onSelectTemplate when template card is clicked", () => {
    ...
    const sources = [createMockSource()];
    render(<TemplateBrowser sources={sources} onSelectTemplate={onSelectTemplate} />);
  ...
  it("toggles view mode between grid and list", () => {
    const sources = [createMockSource()];
    const { container } = render(<TemplateBrowser sources={sources} />);
    ...
    const sources = [
      createMockSource({
        ...
        status: {
          phase: "Ready",
          templates: [
            { name: "t1", displayName: "Template 1", path: "p1" },
          ],
        },
      }),
      createMockSource({
        ...
        status: {
          phase: "Ready",
          templates: [
            { name: "t2", displayName: "Template 2", path: "p2" },
          ],
        },
      }),
    ];
  ...
  it("applies custom className", () => {
    ...
    const { container } = render(
      <TemplateBrowser sources={sources} className="custom-class" />
    );
  ...
  it("disables refresh button when loading", () => {
    const onRefetch = vi.fn();
    const sources = [createMockSource()];
    render(<TemplateBrowser sources={sources} onRefetch={onRefetch} loading />);
