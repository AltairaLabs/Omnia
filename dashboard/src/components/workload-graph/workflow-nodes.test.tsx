...
describe("workflow shape nodes", () => {
  it("renders a stadium state with label and fires onClick", () => {
    const node: WorkloadNode = { id: "s", kind: "state", label: "Plan", badges: [{ icon: "tool", label: "2" }], detail: {} };
    const onClick = vi.fn();
  ...
  it("renders a variable lozenge and an artifact parallelogram", () => {
    wrap(<VariableNode data={{ node: { id: "v", kind: "variable", label: "topic", badges: [], detail: { varType: "string" } } }} />);
    wrap(<ArtifactNode data={{ node: { id: "a", kind: "artifact", label: "notes", badges: [], detail: { artifactMode: "append" } } }} />);
