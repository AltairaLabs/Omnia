-- Audit drain-forwarder bookkeeping (#1673). session-api ships its local audit_log
...
--
-- audit_log is partitioned by "timestamp"; ALTER TABLE ADD COLUMN on the parent
-- cascades to every partition, and the partial index created on the parent is a
-- partitioned index that cascades to each partition too.
