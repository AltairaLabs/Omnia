...
// EvalResult represents a single evaluation result for the facade-side Store interface.
// Written by both the runtime (inline evals) and the arena eval worker.
//
// NOTE: A structurally similar type exists in internal/session/api/eval_store.go
// (api.EvalResult). The two types differ in JSON omitempty tags: this version
// uses omitempty for optional fields (appropriate for the write path where fields
// may be absent), while the api version omits omitempty on required fields so
// that zero values are always serialized in API responses.
...
type Store interface {
	...
	// Returns ErrSessionNotFound if the session does not exist.
	// TODO: Store.GetMessages returns []Message (value) while WarmStoreProvider.GetMessages
	// returns []*Message (pointer). This inconsistency should be unified in a future refactor.
