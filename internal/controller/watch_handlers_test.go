...
func TestReconcileProviderRef(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = omniav1alpha1.AddToScheme(scheme)
	_ = corev1.AddToScheme(scheme)
	...
	tests := []struct {
	...
	}{
		{
			...
			provider: &omniav1alpha1.Provider{
				...
				Status: omniav1alpha1.ProviderStatus{
					Phase: omniav1alpha1.ProviderPhaseReady,
				},
			},
			expectProvider: true,
			expectError:    false,
		...
		},
		{
			...
			provider: &omniav1alpha1.Provider{
				...
				Status: omniav1alpha1.ProviderStatus{
					Phase: omniav1alpha1.ProviderPhaseError,
				},
			},
			expectProvider: false,
			expectError:    false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fakeClient := fake.NewClientBuilder().
				WithScheme(scheme).
				WithRuntimeObjects(objs...).
				WithStatusSubresource(tt.agentRuntime).
				Build()
			...
			log := logr.Discard()
			provider, result, err := r.reconcileProviderRef(ctx, log, tt.agentRuntime)
			...
			if tt.expectProvider {
				...
				assert.Equal(t, tt.provider.Name, provider.Name)
			} else {
