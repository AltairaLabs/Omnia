...
var _ = Describe("PromptPack Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind PromptPack")
			err := k8sClient.Get(ctx, typeNamespacedName, promptpack)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance PromptPack")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &PromptPackReconciler{
				Client: k8sClient,
