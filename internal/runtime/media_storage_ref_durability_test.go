...
//     request-capturing provider, proving the raw StorageReference (not a
//     resolved/frozen byte blob) reaches the provider boundary on BOTH turns.
//
...
// the part just sent on that turn rather than one replayed from history.
...
// provider boundary unresolved — a live reference, not a resolved/frozen
// byte blob — on BOTH turns.
