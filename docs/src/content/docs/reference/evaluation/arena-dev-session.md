...
ArenaDevSession is an enterprise feature. The CRD is only installed when `enterprise.enabled=true` in your Helm values. See [Installing a License](/how-to/operations/install-license/) for details.
:::
...
```mermaid
sequenceDiagram
    participant PE as Project Editor
    participant DS as ArenaDevSession
    participant Pod as Dev Console Pod
    PE->>DS: Click "Test Agent"
    DS->>Pod: Create Pod + Service
    Pod-->>DS: Status: Ready
    DS-->>PE: WebSocket URL
    PE-)Pod: WebSocket connection
    Note over PE,Pod: Idle timeout expires
    DS->>Pod: Delete session (cleanup)
```
...
See [Provider CRD](/reference/core/provider) for configuration details.
...
- **[Project Editor](/how-to/evaluation/use-arena-project-editor)**: Where dev sessions are started
- **[Provider](/reference/core/provider)**: LLM provider configuration
- **[WebSocket Protocol](/reference/platform/websocket-protocol)**: Base WebSocket message types
