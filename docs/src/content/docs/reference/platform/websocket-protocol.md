...
> **Note**: Media upload is only available when the facade is configured with media storage. See [AgentRuntime facade.media configuration](/reference/core/agentruntime/#facadesmedia) for details.
...
```mermaid
flowchart LR
    H["Header<br/>32 bytes"] --- M["Metadata<br/>JSON"] --- P["Binary payload"]
```
