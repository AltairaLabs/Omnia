...
For non-PromptKit agents (Pattern A), the eval worker must be deployed via Helm (see [Eval Worker Helm values](/reference/platform/helm-values/#eval-worker-configuration)):
