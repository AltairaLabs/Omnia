...
External authentication is governed by the agent's `spec.externalAuth` setting (apiKeys, OIDC, or edge trust), not the dashboard's management-plane token. Configure external auth on the AgentRuntime to control how clients authenticate to the external endpoint.
