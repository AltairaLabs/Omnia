...
try {
  ...
  const result = await checker.check({
    ...
    timeout: 10000,
    // Skip patterns:
    //   - *.altairalabs.ai: canonical Starlight URLs (from `site`) plus sibling
    //     AltairaLabs family product sites (promptkit, promptarena, codegen, …)
    //     linked from the masterbrand switcher. These resolve once each product
    //     is deployed; checking them catches stale-deploy false positives, not
    //     real authoring bugs.
    ...
    //   - github.com/.../{edit,blob,tree,security}/...: "Edit this page" links and
    //     repo file/security links redirect to login for unauthenticated crawlers
    //     and flake under rate limiting. The links are valid; just crawler-hostile.
    linksToSkip: [
      '^https?://([a-z0-9-]+\\.)?altairalabs\\.ai',
      '^http://localhost:(?!' + PORT + ')\\d+',
      '^https?://portal\\.azure\\.com',
      '^https?://kind\\.sigs\\.k8s\\.io',
      '^https?://github\\.com/[^/]+/[^/]+/(edit|blob|tree|security)/',
    ],
  });
