...
type MgmtPlaneTokenMinter struct {
	...
	ttl      time.Duration
	now      func() time.Time
	// statFn / readFn are injectable so tests can drive rotation
	// without filesystem juggling. Production wires os.Stat / os.ReadFile.
	statFn func(string) (os.FileInfo, error)
	readFn func(string) ([]byte, error)

	mu     sync.Mutex
	loaded loadedSigningKey
	cached cachedMgmtPlaneToken
}

// loadedSigningKey caches the parsed key + derived kid alongside the
// mtime of the file they came from. We re-stat on every Token() call:
// when Helm regenerates the dashboard signing-keypair Secret (which
// happens on `helm template | kubectl apply` because resource-policy
// annotations only bind helm itself), the kubelet drops the new file
// into Doctor's mounted volume within ~60s. Without re-checking,
// Doctor would mint with the stale in-memory key indefinitely and
// the facade would 401 every WS dial with "unknown JWT kid" — the
...
// NewMgmtPlaneTokenMinter validates the key file at opts.KeyPath and
// returns a minter ready to sign tokens. The kid is the RFC 7638
...
// key during rotation.
//
// The key is read again on every Token() call when the file's mtime
// has changed since last load. Helm's signing-keypair Secret can be
// regenerated by `helm template | kubectl apply` (the resource-policy
// annotation only binds helm itself, not raw kubectl), and the
// kubelet propagates the new bytes to mounted Secrets within ~60s.
// Without re-reading we'd mint with a stale key indefinitely and the
// facade would 401 every WS dial with "unknown JWT kid" — the symptom
...
func NewMgmtPlaneTokenMinter(opts MgmtPlaneTokenMinterOptions) (*MgmtPlaneTokenMinter, error) {
	...
	// Eager initial load so a misconfigured pod fails at boot rather
	// than at first WS dial. Reload semantics match the on-demand path
	// in Token().
	if _, err := m.loadKeyLocked(); err != nil {
		return nil, err
	}
	return m, nil
}

// loadKeyLocked re-reads the key file when its mtime / size has
// changed since the cached version, parses the RSA key, derives the
// kid, and updates m.loaded. Returns the post-call loadedSigningKey
// for the caller to mint with. Caller MUST hold m.mu.
func (m *MgmtPlaneTokenMinter) loadKeyLocked() (loadedSigningKey, error) {
	info, err := m.statFn(m.keyPath)
	if err != nil {
		return loadedSigningKey{}, fmt.Errorf("mgmt-plane minter: stat key %q: %w", m.keyPath, err)
	}
	mtime := info.ModTime()
	size := info.Size()
	// Cached load is still valid when both mtime AND size are
	// unchanged. Some filesystems (notably Kubernetes' Secret mount,
	// which uses atomic symlink swaps) can preserve mtime under edge
	// cases; comparing size as well guards against a same-mtime
	// rotation slipping through.
	if m.loaded.key != nil && m.loaded.mtime.Equal(mtime) && m.loaded.keySize == size {
		return m.loaded, nil
	}

	pemBytes, err := m.readFn(m.keyPath)
	if err != nil {
		return loadedSigningKey{}, fmt.Errorf("mgmt-plane minter: read key %q: %w", m.keyPath, err)
	}
	key, err := parseRSAPrivateKey(pemBytes)
	if err != nil {
		return loadedSigningKey{}, fmt.Errorf("mgmt-plane minter: parse key %q: %w", m.keyPath, err)
	}
	m.loaded = loadedSigningKey{
		key:     key,
		kid:     rsaThumbprint(&key.PublicKey),
		mtime:   mtime,
		keySize: size,
	}
	// A rotated key invalidates the cached token: it was signed by
	// the previous key and carries the previous kid. Without this
	// the minter would happily hand out an unverifiable token until
	// the cache TTL elapsed.
...
func (m *MgmtPlaneTokenMinter) Token(agent, workspace string) (string, error) {
	...
	// Re-stat / reload picks up Helm-driven Secret rotation. On the
	// hot path this is one stat() (~µs) and a no-op when the file
	// hasn't changed; only the rotation itself pays the parse cost.
	loaded, err := m.loadKeyLocked()
	if err != nil {
		return "", err
	}
	...
	token.Header["kid"] = loaded.kid
	signed, err := token.SignedString(loaded.key)
	if err != nil {
