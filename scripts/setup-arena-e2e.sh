...
# and omnia-arena-controller (when workspaceContent.enabled=true). Kind's default
# `standard` storage class uses volumeBindingMode: WaitForFirstConsumer, which
# means the PVC stays Pending until a pod schedules. When two pods race to be
# the first consumer of the same RWO PVC, the scheduler's PreBind optimistic-
# concurrency retry loop can stall for minutes ("object has been modified" on
# the PVC). We side-step this by giving workspaceContent its own StorageClass
# with Immediate binding — the PVC gets a PV at creation time, before any pod
# tries to schedule.
