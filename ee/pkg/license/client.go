...
// licenseWire is the JSON shape served by GET /api/v1/license (the
// dashboard-facing DTO in ee/cmd/omnia-arena-controller/api/server.go, which
// uses id/issuedAt/expiresAt rather than the JWT-style lid/iat/exp tags). Only
// the fields needed to evaluate entitlements are decoded; the Features object
// shares its json tags with license.Features, so it decodes directly.
