-- deletion_requests: DSAR / right-to-erasure lifecycle, relocated from session-api
-- to privacy-api (#1676, Phase 2). privacy-api owns the request lifecycle/status;
-- per-tier deletion is delegated to each service-group's session-api + memory-api.
-- DDL matches the session-api origin table (internal/session/postgres/migrations/
-- 000001_initial.up.sql) so the shared PostgresDeletionStore works unchanged.
