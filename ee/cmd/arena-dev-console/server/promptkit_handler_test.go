...
func TestBuildEngineComponentsOutputDirectory(t *testing.T) {
	...
	registry, _, _, _, _, _, _, _, err := engine.BuildEngineComponents(cfg, nil)
...
func TestPromptKitHandlerCloseWithNamespaceRegistries(t *testing.T) {
	...
	nsRegistry, _, _, _, _, _, _, _, err := engine.BuildEngineComponents(cfg2, nil)
	require.NoError(t, err)
...
func TestGetOrLoadK8sRegistryCached(t *testing.T) {
	...
	cachedRegistry, _, _, _, _, _, _, _, err := engine.BuildEngineComponents(cfg, nil)
	require.NoError(t, err)
	defer func() {
