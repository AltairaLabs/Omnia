...
- **Privacy / compliance audit** (enforcement events — `pii_redacted`, `memory_write_blocked` — opt-out blocks, consent changes, enforcement-stats): **privacy-api is the owner and source of truth** (#1673). memory-api and session-api record enforcement events in their own local `audit_log`, then a drain-forwarder (`ee/pkg/audit/forwarder.go`) ships them at-least-once to privacy-api's central `audit_log` hub (`POST /api/v1/privacy/audit-events`). The dashboard's enforcement-stats read path goes to privacy-api (`GET /api/v1/privacy/enforcement-stats`), not session-api. Rows are keyed by workspace **UID** end-to-end.
- **DSAR / right-to-erasure** (deletion-request lifecycle): **privacy-api owns the lifecycle** (#1676). It stores `deletion_requests` in its own DB and orchestrates erasure across every service-group — calling each session-api's `delete-by-user` (sessions + media) and memory-api's batch-delete (memories, scoped by workspace UID), both SA-authenticated. Per-tier deletion stays with the owning service; privacy-api holds no warm-store/object-storage creds. session-api no longer serves the `deletion-request[s]` routes (its `deletion_requests` table is dropped); it keeps only `delete-by-user`. DSAR lifecycle events (`deletion_requested`/`_completed`/`_failed`) are written to the central `audit_log` (source_service `privacy-api-dsar`, #1678).
...
**`SERVICES.md` (repo root) is the source of truth** — full node/edge topology,
protocol table, tracing/span inventory. This is the condensed view.
...
```
                    ...
                    └──────┬───────┘
                           │ HTTP / WebSocket (external + mgmt twin)
                    ┌──────┴───────┐
            ...
            │  Agent Pod                    │
            │  ┌──────────┐ gRPC ┌────────┐ │       ┌──────────────┐
            │  │ Facade   │─────▶│Runtime │ │──HTTP▶│ Session API  │──▶ Postgres
            ...
            └────────────────────│Memory  │─┘       │ (EE)         │    (from memory-api)
                                 │API cmd/│─────────│ ee/cmd/      │──▶ DSAR fan-out
                                 │memory- │  audit  │ privacy-api/ │    back to session-
                                 │api/    │  drain  └──────────────┘    api + memory-api
                                 └────────┘
```
